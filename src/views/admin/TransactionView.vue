<template>
  <div class="flex flex-col justify-center items-center">
    ...
    <i
      @click.prevent="back"
      class="-mt-90 lg:block md:block hidden ri-arrow-left-circle-fill fixed lg:left-64 md:left-44 sm:left-28 text-sky-600 font-medium text-lg hover:text-2xl hover:text-sky-300 active:text-sky-700 text-shadow duration-500"
    ></i>
      ...
      <div class="flex justify-between mb-4 ml-4">
        <div class="gap-1 grid grid-flow-col">
          <button @click="dropdownClass" class="relative flex justify-center items-center bg-orange-500 active:bg-orange-600 text-white rounded">
            <p class="px-1 text-xxs lg:text-xs md:text-xs">Class</p>
            <span class="border-l p-1 hover:bg-orange-300 rounded text-xxs lg:text-xs md:text-xs"><i class="ri-arrow-down-circle-fill"></i></span>
            <div v-if="toggleClass" class="rounded absolute top-full min-w-full w-max bg-white shadow-md mt-1 text-black">
              ...
              <ul class="text-left border rounded" v-for="(item, index) in kelas" :key="index">
                <li @click="filterClass(item.kelas)" class="text-xxs lg:text-xs md:text-xs px-2 py-1 border-b active:bg-orange-400 active:text-white">{{ item.kelas }}</li>
              </ul>
            </div>
          </button>
          <button @click="dropdownMajor" class="relative flex justify-center items-center bg-emerald-500 active:bg-emerald-600 text-white rounded">
            <p class="px-1 text-xxs lg:text-xs md:text-xs">Major</p>
            <span class="border-l p-1 hover:bg-emerald-300 rounded text-xxs lg:text-xs md:text-xs"><i class="ri-arrow-down-circle-fill"></i></span>
            <div v-if="toggleMajor" class="rounded absolute top-full min-w-full w-max bg-white shadow-md mt-1 text-black max-h-40 overflow-y-auto">
              ...
              <ul class="text-left border rounded" v-for="(item, index) in majors" :key="index">
                <li @click="filterMajor(item.id)" class="text-xxs lg:text-xs md:text-xs px-2 py-1 border-b active:bg-emerald-400 active:text-white">{{ item.major_name }}</li>
              </ul>
            </div>
          </button>
        </div>
...
<script>
import Nav from '@/components/admin/navigasi/NavAdmin7.vue';
import { useRouter } from 'vue-router';
import TableStudent from '@/components/admin/table/TableStudentOrder.vue';
...
export default {
  ...
  setup() {
    ...
    return { dropdownClass, dropdownMajor, ...toRefs(data), toggleClass, toggleMajor, paginate, majors, filter, filterClass, filterMajor, getStudents, back };
  },
};
</script>
