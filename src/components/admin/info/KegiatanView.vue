<template>
  <div>
    ...
    ></i>
    <div class="flex flex-wrap mt-8 justify-evenly lg:mt-24 md:mt-24 lg:ml-64 md:ml-48">
      <div class="w-full px-4 gap-1 mb-3 flex">
        <button @click="toggleCreate('createKegiatan')" class="px-2 bg-sky-500 hover:bg-sky-300 active:bg-sky-600 uppercase text-white rounded">
      ...
      <div v-for="(item, index) in news" :key="index" class="w-full flex flex-col justify-center items-center px-3 mb-5">
        ...
        <div class="w-full lg:w-1/2 md:w-1/2 bg-white shadow-lg flex flex-col px-4 lg:px-10 md:px-7 py-3 rounded-b-md">
          <div class="flex justify-between my-3 font-medium">
            ...
            <span class="text-rose-500 text-xxs lg:text-sm md:text-sm w-1/3 text-right">{{ item.date }}</span>
          </div>
          <p class="text-xs lg:text-sm md:text-sm mt-2 mb-5 text-slate-700">{{ item.body }}</p>

          <div class="flex w-full justify-center gap-1 mb-4 text-xs lg:text-base md:text-sm">
...
import Modal from '@/components/admin/modal/ModalKegiatan.vue';
...
export default {
  ...
  setup() {
    ...
    const drop = async (value) => {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          const res = await axios.post(`/api/info/delete/${value.id}`);
          if (res.status == 200) {
            alert(res.data.meassage);
            store.commit('info/dropKegiatan', value);
            getKegiatan();
          }
        }
      });
    ...
    const set = (value) => {
      store.commit('info/setModal', 'setKegiatan');
      store.commit('info/setToggleKegiatan');
      store.commit('info/getKegiatan', value);
    };
