<template>
  <div class="overflow-x-auto">
    <table class="w-full text-center text-xxs lg:text-xs md:text-xs">
      <tr class="bg-stone-800 text-white w-full">
        <th class="p-3"><i class="ri-pushpin-fill"></i></th>
        <th class="p-3">Nama Jurusan</th>
        <th class="p-3">Options</th>
      </tr>
      <!-- thebody -->
      <tr class="odd:bg-slate-200  bg-white" v-for="(item,index) in majors" :key="index">
        <td class="p-3 font-medium">{{index+1}}</td>
        <td class="p-3 font-medium">{{item.major_name}}</td>
        <td class="flex justify-center p-3 gap-1 text-white">
          <button @click="set(item)" class="bg-orange-400 px-3 py-1 lg:py-2 md:py-2 rounded shadow-md active:bg-orange-500 hover:bg-orange-300"><i class="ri-settings-4-fill"></i></button>
          <button @click="drop(item)" class="bg-rose-500 px-3 py-1 lg:py-2 md:py-2 rounded shadow-md active:bg-rose-600 hover:bg-rose-400"><i class="ri-delete-bin-7-fill"></i></button>
        </td>
      </tr>
    </table>
  </div>
</template>
<script>
import { useStore } from 'vuex';
import { computed } from '@vue/runtime-core';
import axios from 'axios';
import Swal from 'sweetalert2'
export default {
  setup() {
    const store = useStore();
    const majors = computed(() => {
      return store.state.admin.majors;
    });

    // function set 
     const set = (value) =>{
      store.commit('admin/getMajor',value);
      store.commit('option/setModal','setMajor');
      store.commit('option/majorToggle');
     }

     const drop = async (value) => {
      // let tanya = confirm('Apakah anda yakin?');
      Swal.fire({
      title: 'Apakah kamu yakin?',
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#3085d6',
      cancelButtonColor: '#d33',
      confirmButtonText: 'Yes, delete it!',
      customClass:{
                    icon:'text-xs',
                    title:'text-mini',
                   }
    }).then( async(result) => {
      if (result.isConfirmed) {
       await store.dispatch('admin/dropMajor', value);
        const response = await axios.get('api/major/majors')
        store.commit('admin/getMajors', response.data.data);
        pagination(response.data.meta, response.data.links);
      }
    })





      
      };
    // function set 

    const pagination = (meta, links) => {
          let data = {
          current_page: meta.current_page,
          last_page: meta.last_page,
          next: links.next,
          prev: links.prev,
          };
          store.commit('admin/setPaginateMajor', data);
        };

    return {majors, set, drop}
  }
};
</script>
<style>
.text-mini{
  font-size:15px;
}
</style>