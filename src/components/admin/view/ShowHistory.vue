<template>
  ...
  <i @click.prevent="back" class="-mt-90 lg:block md:block hidden ri-arrow-left-circle-fill fixed  lg:left-64 md:left-44 sm:left-28  text-sky-600 font-medium text-lg hover:text-2xl  hover:text-sky-300 active:text-sky-700  text-shadow duration-500"></i>
    ...
    <div class="flex flex-wrap mt-8 justify-start  lg:items-center md:item-center lg:mt-24  md:mt-24 lg:ml-56 md:ml-56">
      <div v-for="(item, index) in wadah" :key="index" class="w-1/2 flex flex-col justify-center items-center px-3 mb-5 mt-5">
        <div class="w-full lg:w-1/2 md:w-3/4 bg-white py-4 px-3 lg:px-5 shadow-lg rounded-md rounded-bl-md flex flex-col">
          ...
          <button @click.prevent="update" class="text-xs uppercase lg:text-base md:text-sm bg-emerald-500 text-center text-white py-1 rounded hover:bg-emerald-300 active:bg-emerald-500"><span class="cursor-pointer">Lunas</span></button>
        </div>
      </div>
    </div>
      <div v-if="paginate.prev || paginate.next" class="w-full px-4 text-white mt-5 text-xxs lg:w-2/5 lg:ml-96 md:ml-72 lg:text-sm md:text-sm">
...
export default {
  ...
  setup() {
    ...
    const drop = async (value) => {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          await axios.post(`/api/frontend/${value.id}`);
          getHistory();
        }
      });
    };
    ...
    const router = useRouter()
         const back = () =>{
             router.replace({name:'admin-history'})
         }
