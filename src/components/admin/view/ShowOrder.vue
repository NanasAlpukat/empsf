    ...
    <div class="flex flex-wrap mt-8 justify-start lg:items-center md:item-center lg:mt-24 md:mt-24 lg:ml-56 md:ml-56">
      <div class="w-1/2 flex flex-col justify-center items-center px-3 mb-5" v-for="(item, index) in pending" :key="index">
        <div class="w-full lg:w-1/2 md:w-3/4 bg-white p-3 lg:p-5 md:p-4 shadow-lg rounded-md rounded-bl-md flex flex-col">
          ...
          <span @click="update" v-if="item.status == 'belum lunas'" class="text-2xs lg:text-sm md:text-sm mt-2 bg-amber-400 text-center text-white py-1 lg:py-2 md:py-2 rounded hover:bg-amber-200 active:bg-amber-400 uppercase cursor-pointer"
...
export default {
  ...
  setup() {
    ...
    const update = () => {
      getPending();
      router.replace({ name: 'show-admin-order' });
    };
    ...
    const back = () => {
      router.replace({ name: 'payment-transaction' });
    };
